...
def upgrade() -> None:
    ...
    op.add_column('job_routings', sa.Column('claimed_at', sa.DateTime(timezone=True), nullable=True))
    
    # Create index for claimed_at field
    op.create_index('idx_job_routings_claimed_at', 'job_routings', ['claimed_at'])
    
    # Add PROCESSING status to sync_status_enum
    op.execute("ALTER TYPE sync_status_enum ADD VALUE 'processing'")
    
    # Add unique constraint to prevent duplicate routings
    op.create_unique_constraint('uq_job_routings_job_company', 'job_routings', ['job_id', 'company_id_received'])
