...
class InMemoryJobQueue(JobQueueInterface):
    ...
    async def dequeue(self, queue_name: str) -> Optional[dict]:
        ...
        logger.info("Job dequeued", job_id=job_id, priority=priority)

        return {"id": job_id, "data": job_data, "priority": priority}
    ...
    async def cancel_job(self, job_id: str) -> bool:
        ...
        self.queue = [(p, jid, data) for p, jid, data in self.queue if jid != job_id]
...
class RedisJobQueue(JobQueueInterface):
    ...
    async def enqueue(self, job_data: dict, priority: int = 0) -> str:
        ...
        try:
            ...
            logger.info("Job enqueued in Redis", job_id=job_id, priority=priority)

            return job_id

        except Exception as e:
            logger.error("Failed to enqueue job in Redis", error=str(e))
            raise
    ...
    async def dequeue(self, queue_name: str) -> Optional[dict]:
        ...
        try:
            ...
            if job_data:
                ...
                logger.info("Job dequeued from Redis", job_id=job_id)
            ...
            logger.error("Failed to dequeue job from Redis", error=str(e))
            return None
    ...
    async def get_job_status(self, job_id: str) -> dict:
        ...
        try:
            ...
            logger.error(
                "Failed to get job status from Redis", job_id=job_id, error=str(e)
            )
            return {"status": "error"}
    ...
    async def cancel_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            logger.error("Failed to cancel job in Redis", job_id=job_id, error=str(e))
            return False
