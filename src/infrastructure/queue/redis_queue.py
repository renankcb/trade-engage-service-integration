...
class RedisQueue:
    ...
    async def enqueue(
        ...
        try:
            ...
            logger.error(
                "Failed to enqueue task in Redis",
                queue=queue_name,
                error=str(e)
            )
            raise
        ...
        try:
            ...
            if task_data:
                ...
                return {
                    "id": task_id,
                    "data": json.loads(
                        task_data.get(b"data", "{}")
                    ),
                    "priority": int(
                        task_data.get(b"priority", 0)
                    )
                }
            ...
            logger.error(
                "Failed to dequeue task from Redis",
                queue=queue_name,
                error=str(e)
            )
            return None
    ...
    async def get_task_status(self, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Failed to get task status from Redis",
                task_id=task_id,
                error=str(e)
            )
            return {"status": "error"}
    ...
    async def cancel_task(self, task_id: str, queue_name: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to cancel task in Redis",
                task_id=task_id,
                error=str(e)
            )
            return False
    ...
    async def clear_queue(self, queue_name: str) -> int:
        ...
        try:
            ...
            logger.error(
                "Failed to clear queue in Redis",
                queue=queue_name,
                error=str(e)
            )
            return 0
