...
class ServiceTitanProvider(ProviderInterface):
    ...
    def __init__(self, company: Company):
        ...
        if not self._validate_config():
            raise ProviderConfigurationError(
                "Invalid ServiceTitan configuration"
            )
    ...
    async def _check_existing_lead(self, idempotency_key: str) -> Optional[Dict[str, Any]]:
    ...
    async def get_lead_status(self, external_id: str) -> dict:
        ...
        try:
            ...
            raise ProviderAPIError(
                f"Failed to get lead status: {str(e)}"
            )
    ...
    async def batch_get_job_status(
        ...
        try:
            ...
            raise ProviderAPIError(
                f"Failed to batch get job status: {str(e)}"
            )

    async def _get_batch_status(self, external_ids: List[str]) -> List[JobStatusResponse]:
    ...
    async def update_lead(self, external_id: str, update_data: dict) -> bool:
        ...
        try:
            ...
            logger.info(
                "Lead updated successfully in ServiceTitan",
                external_id=external_id
            )
