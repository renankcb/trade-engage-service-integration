...
class RetryHandlerInterface(ABC):
    ...
    @abstractmethod
    async def get_delay(self, attempt: int, base_delay: float, backoff_factor: float) -> float:
        """Calculate delay before next retry attempt."""
        pass
    
    @abstractmethod
    async def record_attempt(self, operation_id: str, attempt: int, success: bool) -> None:
