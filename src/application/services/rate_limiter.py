...
class RedisRateLimiter(RateLimiterInterface):
    ...
    async def check_rate_limit(self, key: str, max_requests: int = 100, window_seconds: int = 60) -> bool:
        ...
        try:
            ...
            # Get current count
            current_key = f"rate_limit:{key}:{datetime.utcnow().strftime('%Y%m%d%H%M')}"
...
class RateLimiter:
    ...
    async def check_and_increment(self, key: str, max_requests: int = 100, window_seconds: int = 60) -> bool:
