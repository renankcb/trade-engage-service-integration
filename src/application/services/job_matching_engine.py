...
class JobMatchingEngine:
    ...
    async def find_matching_company(
        self,
        ...
        """
        Find the BEST company that matches the job requirements.

        Args:
            job_requirements: Job requirements and constraints
            available_companies: List of available companies with their data
            exclude_company_id: Company ID to exclude from matching (e.g., requesting company)

        Returns:
            Single CompanyMatch object with the highest score, or None if no matches
        """
        self.logger.info(
            "Finding best matching company for job",
            job_id=str(job_requirements.job_id),
            required_skills=job_requirements.required_skills,
            exclude_company_id=str(exclude_company_id) if exclude_company_id else None,
        )

        matches = []

        for company in available_companies:
            if company["id"] == exclude_company_id:
                continue
        ...
        if not matches:
            self.logger.warning(
                "No matching companies found for job",
                job_id=str(job_requirements.job_id),
                required_skills=job_requirements.required_skills,
        ...
        # If there are multiple companies with the same score, return the first one
        same_score_matches = [m for m in matches if m.score == best_match.score]
        if len(same_score_matches) > 1:
            self.logger.info(
                "Multiple companies with same score, selecting first one",
                job_id=str(job_requirements.job_id),
                score=best_match.score,
                companies_count=len(same_score_matches),
            )

        self.logger.info(
            "Found best matching company",
            job_id=str(job_requirements.job_id),
            company_id=str(best_match.company_id),
            score=best_match.score,
    ...
    def _calculate_match_score(
        self, job_requirements: JobRequirements, company: Dict[str, Any]
        ...
        primary_skills = [
            ...
            if company.get("is_primary_skill", {}).get(skill, False)
        ]
    ...
    def _calculate_skill_level_score(
        self, required_level: str, company_level: str
    ) -> float:
    ...
    def _calculate_location_score(
        self, job_location: Dict[str, str], company_location: Dict[str, str]
    ) -> float:
    ...
    async def get_job_requirements(self, job_id: UUID) -> JobRequirements:
        ...
        return JobRequirements(job_id=job_id, required_skills=[], skill_levels={})
