...
class OutboxWorker:
    ...
    async def _process_job_sync_event(self, event: OutboxEvent) -> bool:
        ...
        try:
            ...
            if not routing_id:
                self.logger.error(
                    "Missing routing_id in job sync event",
                    event_id=str(event.id)
                )
                return False
            ...
            self.logger.error(
                "Failed to process job sync event",
                event_id=str(event.id),
                error=str(e)
            )
            return False
    ...
    async def _process_company_sync_event(self, event: OutboxEvent) -> bool:
        ...
        self.logger.info(
            "Company sync event processing not yet implemented",
            event_id=str(event.id)
        )
        return True
    ...
    async def _process_provider_sync_event(self, event: OutboxEvent) -> bool:
        ...
        self.logger.info(
            "Provider sync event processing not yet implemented",
            event_id=str(event.id)
        )
        return True
    ...
    async def start_continuous_processing(self, interval_seconds: int = 30):
        ...
        while self.is_running:
            try:
                ...
                self.logger.error(
                    "Error in continuous outbox processing",
                    error=str(e),
                    exc_info=True
                )
                await asyncio.sleep(interval_seconds)
