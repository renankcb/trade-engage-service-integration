...
@current_app.task(bind=True, max_retries=2, name="cleanup_completed_jobs_task")
    ...
    try:
        ...
        for job in old_completed_jobs:
            try:
                ...
                job_routings = asyncio.run(job_routing_repo.find_by_job_id(job.id))
                ...
                logger.error("Error cleaning up job", job_id=str(job.id), error=str(e))
...
@current_app.task(bind=True, max_retries=2, name="cleanup_failed_jobs_task")
...
@current_app.task(bind=True, max_retries=1, name="cleanup_outbox_events_task")
def cleanup_outbox_events_task(self):
    """Clean up old completed outbox events."""
    try:
        logger.info(
            "Starting outbox events cleanup task", attempt=self.request.retries + 1
        )
        ...
        # Clean up old events
        cleaned_count = asyncio.run(outbox.cleanup_completed_events(cutoff_date))
...
@current_app.task(bind=True, max_retries=1, name="cleanup_orphaned_routings_task")
def cleanup_orphaned_routings_task(self):
    """Clean up job routings that reference non-existent jobs."""
    try:
        logger.info(
            "Starting orphaned routings cleanup task", attempt=self.request.retries + 1
        )
        ...
        logger.info(
            "Orphaned routings cleanup finished", routings_cleaned=routings_cleaned
        )

        return {"status": "success", "routings_cleaned": routings_cleaned}
